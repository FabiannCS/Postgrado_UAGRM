...
export function ProgramCard({ program, onViewDetails }: ProgramCardProps) {
  return (
    <Card className={`overflow-hidden border-l-4 hover:shadow-lg transition-all duration-300 flex flex-col h-full ${
        program.theme === 'orange' ? 'border-l-orange-500' : 
        program.status === 'Completado' ? 'border-l-emerald-500' : 'border-l-uagrm-blue'
    }`}>
        <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
            <div className="flex items-start gap-4">
                <div className={`p-2.5 rounded-xl shadow-sm ${
                    program.status === 'Completado' ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/20' :
                    program.theme === 'orange' ? 'bg-orange-100 text-orange-600 dark:bg-orange-900/20' : 
                ...
                <div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                        ...
                        <Badge className={`${
                             program.status === 'Activo' ? 'bg-blue-100 text-blue-700 hover:bg-blue-200 dark:bg-blue-900/30 dark:text-blue-300' : 
        ...
        <CardContent className="space-y-4 flex-1 pt-2">
            ...
            <div className="space-y-2 pt-2">
                <div className="flex justify-between items-center text-sm font-medium">
                    ...
                    <span className={`${
                        program.status === 'Completado' ? 'text-emerald-600' :
                        program.theme === 'orange' ? 'text-orange-600' : 'text-uagrm-blue'
                    }`}>
                        {program.progress}%
                    </span>
                </div>
